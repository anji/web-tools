...
/**
 * Initialisms Go convention wants fully capitalised: `ID`, not `Id`; `URL`, not
 * `Url`. Taken from the list golint enforced. Getting these wrong is the most
 * visible tell that a struct was machine-generated by something that does not
 * know Go.
 */
...
/** Splits an arbitrary key into lowercase word parts. */
...
export function snakeCase(input: string): string {
  const joined = words(input).join('_');
  return /^[0-9]/.test(joined) ? `_${joined}` : joined;
}

/** PascalCase with Go's initialism convention applied. */
export function goCase(input: string): string {
  const joined = words(input)
    .map((w) => {
      const upper = w.toUpperCase();
      return GO_INITIALISMS.has(upper) ? upper : w.charAt(0).toUpperCase() + w.slice(1);
    })
    .join('');
  return /^[0-9]/.test(joined) || joined === '' ? `F${joined}` : joined;
}
